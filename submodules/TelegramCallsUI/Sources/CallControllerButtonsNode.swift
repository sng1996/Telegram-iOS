...
final class CallControllerButtonsNode: ASDisplayNode {
    ...
    private var buttonStyle: UIBlurEffect.Style
    ...
    init(strings: PresentationStrings, style: UIBlurEffect.Style = .light) {
    ...
    func cameraButtonNode() -> CallControllerButtonItemNode? {
        buttonNodes[.enableCamera]
    }
    ...
    private func updateButtonsLayout(strings: PresentationStrings, mode: CallControllerButtonsMode, width: CGFloat, bottomInset: CGFloat, animated: Bool) -> CGFloat {
        ...
        let largeButtonSize: CGFloat = 56
        ...
        case .outgoingRinging:
            ...
            var bottomButtonsLeftOffset = floor((width - bottomButtonsWidth) / 2.0)
            for button in bottomButtons {
                buttons.append(PlacedButton(button: button, frame: CGRect(origin: CGPoint(x: bottomButtonsLeftOffset, y: 0), size: CGSize(width: largeButtonSize, height: largeButtonSize))))
                bottomButtonsLeftOffset += largeButtonSize + bottomButtonsSpacing
            }

            height = largeButtonSize + max(bottomInset + 32.0, 46.0)
        case .active:
            if videoState.hasVideo {
                let isCameraActive: Bool
                let isScreencastActive: Bool
        ...
        for button in buttons {
            ...
            if let current = self.buttonNodes[button.button.key] {
                ...
                buttonNode = CallControllerButtonItemNode(style: buttonStyle)
