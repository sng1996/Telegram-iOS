...
final class CallCameraPreviewController: ViewController {
    ...
    override public func loadDisplayNode() {
        self.displayNode = CallCameraPreviewControllerNode(controller: self, sharedContext: self.sharedContext, cameraNode: self.cameraNode)
...
//        self.controllerNode.animateIn(fromRect: fromRect)
//    }
//
//    func animateOut(toRect: CGRect, toCornerRadius: CGFloat) {
//        self.controllerNode.animateOut(toRect: toRect, toCornerRadius: toCornerRadius)
//    }
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        self.controllerNode.containerLayoutUpdated(layout, navigationBarHeight: 0, transition: transition)
    }
}
...
private class CallCameraPreviewControllerNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    private var wheelNode: WheelControlNodeV2
    ...
    init(controller: CallCameraPreviewController, sharedContext: SharedAccountContext, cameraNode: PreviewVideoNode) {
        ...
        self.titleNode.attributedText = NSAttributedString(string: title, font: .systemFont(ofSize: 17, weight: .semibold), textColor: UIColor(rgb: 0xffffff))

        self.doneButton = SolidRoundedButtonNode(theme: SolidRoundedButtonTheme(backgroundColor: UIColor(rgb: 0xffffff), foregroundColor: UIColor(rgb: 0x4f5352)), font: .bold, height: 50, cornerRadius: 10, gloss: false)
        ...
        self.wheelNode = WheelControlNodeV2(items: [WheelControlNodeV2.Item(title: UIDevice.current.model == "iPad" ? self.presentationData.strings.VoiceChat_VideoPreviewTabletScreen : self.presentationData.strings.VoiceChat_VideoPreviewPhoneScreen), WheelControlNodeV2.Item(title: self.presentationData.strings.VoiceChat_VideoPreviewFrontCamera), WheelControlNodeV2.Item(title: self.presentationData.strings.VoiceChat_VideoPreviewBackCamera)], selectedIndex: self.selectedTabIndex)
    ...
    func animateIn(/*fromRect: CGRect*/) {
//        let maskView = UIView(frame: fromRect)
//        maskView.backgroundColor = .white
//        maskView.layer.cornerRadius = fromRect.height / 2
//        self.view.mask = maskView
//
//        CATransaction.begin()
//        let animation = CABasicAnimation(keyPath: "cornerRadius")
//        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.linear)
//        animation.fromValue = fromRect.height / 2
//        animation.toValue = 0
...
//            self?.view.mask = nil
//        }
//        maskView.layer.add(animation, forKey: "cornerRadius")
//        CATransaction.commit()
//
//        maskView.layer.animateFrame(
//            from: fromRect,
//            to: self.view.bounds,
//            duration: 0.3)
    ...
    func animateOut(/*toRect: CGRect, toCornerRadius: CGFloat, */completion: (() -> Void)? = nil) {
...
//        self.cameraNode.updateLayout(
//            size: toRect.size,
//            layoutMode: .fillVertical,
//            transition: .animated(duration: 0.3, curve: .linear))
//
//        self.clipsToBounds = true
//        self.layer.animateFrame(
//            from: self.frame,
//            to: CGRect(origin: CGPoint(x: 200, y: 200), size: toRect.size),
//            duration: 0.3)

//        self.layer.animateScaleY(from: 1, to: toRect.height / self.frame.height, duration: 0.3)
//        self.layer.animateScaleX(from: 1, to: toRect.width / self.frame.width, duration: 0.3)
//        self.layer.animatePosition(from: self.position, to: toRect.center, duration: 0.3)
//
//        CATransaction.begin()
//        let animation = CABasicAnimation(keyPath: "cornerRadius")
//        animation.timingFunction = CAMediaTimingFunction(name: CAMediaTimingFunctionName.linear)
...
//        CATransaction.commit()
//
//        self.cancelButton.layer.animateAlpha(from: 1, to: 0, duration: 0.15)
//        self.titleNode.layer.animateAlpha(from: 1, to: 0, duration: 0.15)
//        self.wheelNode.layer.animateAlpha(from: 1, to: 0, duration: 0.15)
//        self.doneButton.layer.animateAlpha(from: 1, to: 0, duration: 0.15)
...
//        insets.top = max(10.0, insets.top)
        ...
        let titleFrame = CGRect(origin: CGPoint(x: floor((contentFrame.width - titleSize.width) / 2.0), y: layout.safeInsets.top + 26.0), size: titleSize)
        transition.updateFrame(node: self.titleNode, frame: titleFrame)

        var previewSize: CGSize
        var previewFrame: CGRect
        let previewAspectRatio: CGFloat = 1
        if isLandscape {
        ...
        let cancelButtonFrame = CGRect(origin: CGPoint(x: 16, y: layout.safeInsets.top + 26.0), size: cancelButtonSize)
        ...
        let bottomButtonInset: CGFloat = 48
        ...
        transition.updateFrame(node: self.doneButton, frame: CGRect(x: floorToScreenPixels((contentFrame.width - buttonWidth) / 2.0), y: previewFrame.maxY - doneButtonHeight - bottomButtonInset, width: buttonWidth, height: doneButtonHeight))
        self.broadcastPickerView?.frame = self.doneButton.frame

        let wheelFrame = CGRect(
            origin: CGPoint(
                x: 16.0 + previewFrame.minX,
                y: self.doneButton.frame.minY - 36.0 - 20.0),
            size: CGSize(
                width: previewFrame.width - 32.0,
                height: 36.0))
...
private class WheelControlNodeV2: ASDisplayNode, UIGestureRecognizerDelegate {
        ...
        if !self.itemNodes.isEmpty {
            ...
            for i in 0 ..< self.itemNodes.count {
                ...
                if itemNode.isSelected != isSelected {
                    ...
                    itemNode.setTitle(title, with: isSelected ? selectedTextFont : textFont, with: isSelected ? .white : UIColor.white.withAlphaComponent(0.5), for: .normal)
                    if isSelected {
